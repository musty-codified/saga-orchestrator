...
/**
 * Relays rows from the local {@code payments_outbox} table to Kafka. Mirrors
 * musty-inventory-service's OutboxRelayService — see that class for the two-tiered
 * transient-vs-poison-row retry rationale. Kept as a separate implementation (rather than a
 * shared abstraction) because the underlying DDL differs by dialect (JSONB/BIGSERIAL here vs
 * JSON/AUTO_INCREMENT there) and this is only the second usage of the pattern.
 */
@Service
...
public class PaymentOutboxRelayService {
    ...
    private static final String DLQ_TOPIC = "PAYMENT_OUTBOX_DLQ_TOPIC";
    ...
    public List<OutboxRow> fetchPending() {
        return postgresJdbcTemplate.query(
                "SELECT id, aggregate_type, aggregate_id, event_type, topic, payload::text AS payload, retry_count " +
                        "FROM payments_outbox WHERE status = 'PENDING' " +
    ...
    public void markPublished(OutboxRow row) {
        postgresJdbcTemplate.update(
                "UPDATE payments_outbox SET status = 'PUBLISHED', published_at = NOW() WHERE id = ?", row.getId());
        log.info("Payments outbox row published → id: {}, topic: {}, aggregateId: {}", row.getId(), row.getTopic(), row.getAggregateId());
    }
    ...
    public void markFailedAttempt(OutboxRow row, Throwable error) {
        ...
        if (nextRetryCount >= MAX_RETRIES) {
            postgresJdbcTemplate.update(
                    "UPDATE payments_outbox SET status = 'FAILED', retry_count = ?, last_error = ? WHERE id = ?",
                    nextRetryCount, truncate(errorMessage), row.getId());
            log.error("Payments outbox row exhausted retries — moving to DLQ → id: {}, topic: {}, aggregateId: {}",
        ...
        postgresJdbcTemplate.update(
                "UPDATE payments_outbox SET retry_count = ?, next_attempt_at = NOW() + (? || ' seconds')::interval, last_error = ? " +
                        "WHERE id = ?",
                nextRetryCount, backoffSeconds, truncate(errorMessage), row.getId());
        log.warn("Payments outbox row publish failed, will retry in {}s → id: {}, topic: {}, attempt: {}",
    ...
    private void publishToDlq(OutboxRow row, String errorMessage) {
        try {
            ...
            log.error("Failed to publish poison payments_outbox row to DLQ (best-effort) → id: {}", row.getId(), dlqPublishFailure);
        }
    }
